...
export default function CBUPage() {
  ...
  async function handleDeposit() {
    if (!canEdit) {
      toast.error('You do not have permission to post CBU transactions');
      return;
    }
  ...
  function handlePrint() {
    ...
    const rows = filtered.map(acc => {
      ...
      return `<tr>
        ...
        <td style="text-align:right;font-weight:600;color:#065f46">${fmt(acc.balance)}</td>
    ...
    const html = `
      <h1 class="report-title">CBU Monitoring</h1>
      <div class="report-meta">Capital Build-Up accounts &nbsp;|&nbsp; Generated: ${new Date().toLocaleString('en-PH')}</div>
      <table>
    ...
    `;
    const win = printHtmlDocument(wrapWithLetterhead(html, {title:'CBU Monitoring — WELLSERVE'}), {
