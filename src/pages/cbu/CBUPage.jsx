...
import { PiggyBank, Search, Eye, TrendingUp, Users, DollarSign, Plus } from 'lucide-react';
...
export default function CBUPage() {
  ...
  const [depositTarget, setDepositTarget] = useState(null); // { account, member }
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6 mb-6">
        <SummaryCard icon={<DollarSign size={20} className="text-green-600" />}  label="Total CBU Balance"  value={formatCurrency(totalBalance)}  bg="bg-green-50" />
        <SummaryCard icon={<TrendingUp size={20} className="text-blue-600" />}   label="Total Deposits"     value={formatCurrency(totalDeposits)} bg="bg-blue-50" />
        <SummaryCard icon={<Users size={20} className="text-purple-600" />}      label="Active Accounts"    value={activeCount}                   bg="bg-purple-50" />
      </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.length === 0 ? (
                  ...
                  <tr key={account.id} className="hover:bg-[#D6FADC]/20 transition-colors">
                    ...
                    <td className="px-4 py-3 font-semibold text-green-700">{formatCurrency(account.balance || 0)}</td>
